<script lang="ts">
	...
	function generateBitWave(bit: number, startX: number, bitWidth: number): [number, number][] {
		...
		const numPoints = 40;
		const amplitude = bit === 1 ? waveAmplitude : waveAmplitude * 0.15;

		for (let i = 0; i <= numPoints; i++) {
			const x = startX + (i / numPoints) * bitWidth;
			const t = (i / numPoints) * Math.PI * 4;
			const y = waveY - amplitude * Math.sin(t);
			points.push([x, y]);
		}
		return points;
	}
	...
	function applyStep(step: number) {
		...
		// Show bits and waves up to current step
	...
	onMount(() => {
		...
		svgEl.append('text')
			...
			.attr('fill', colors.fg)
			.attr('font-size', '14px')
			.text('ASK: Amplitude Shift Keying');
		...
		svgEl.append('text')
			...
			.attr('fill', colors.fgMuted)
			.attr('font-size', '11px')
			.text('1 = full amplitude  •  0 = low amplitude');
