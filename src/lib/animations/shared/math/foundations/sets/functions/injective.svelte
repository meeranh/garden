<script lang="ts">
	...
	let display = $derived.by(() => {
		if (currentStep === 0) {
			...
			return {
				title: 'Injective function',
				showArrows: true,
				message: 'Different inputs → different outputs'
			};
		}
	});
	...
	const leftX = 60;
	const rightX = 240;
	const domainStartY = 45;
	const codomainStartY = 35;
	const gapY = 30;
...
<div class="container">
	...
	<div class="diagram-container">
		<svg viewBox="0 0 300 160" class="diagram">
			<!-- Domain box -->
			<rect x="20" y="20" width="80" height="115" rx="8" class="set-box" />
			<text x="60" y="12" class="set-label">A</text>

			<!-- Codomain box -->
			<rect x="200" y="20" width="80" height="115" rx="8" class="set-box" />
			<text x="240" y="12" class="set-label">B</text>

			<!-- Domain elements -->
			{#each domain as el}
				<circle cx={leftX} cy={getLeftY(el)} r="14" class="element-circle" />
				<text x={leftX} y={getLeftY(el) + 5} class="element-text">{el}</text>
			{/each}

			<!-- Codomain elements -->
			{#each codomain as el}
				<circle cx={rightX} cy={getRightY(el)} r="14" class="element-circle" />
				<text x={rightX} y={getRightY(el) + 5} class="element-text">{el}</text>
			{/each}
