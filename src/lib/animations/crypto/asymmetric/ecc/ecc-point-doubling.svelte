<script lang="ts">
  ...
  // Point P for doubling (x=-1 keeps all points in visible bounds)
  const P = { x: -1, y: curveY(-1) };

  // Tangent slope: m = (3x² + a) / (2y)
  const m = (3 * P.x * P.x + a) / (2 * P.y);
  const c = P.y - m * P.x;

  // Result of doubling using Vieta's formulas: x₃ = m² - 2x₁
  // R' is the intersection point on the line, R is its reflection
  const Rprime_x = m * m - 2 * P.x;
  const Rprime_y = m * Rprime_x + c;
  const Rprime = { x: Rprime_x, y: Rprime_y };
  const R = { x: Rprime.x, y: -Rprime_y };

  const allElements = ['axes', 'curve-upper', 'curve-lower', 'point-p', 'tangent', 'point-rprime', 'reflect-line', 'point-r', 'result'];

  const stepVisibility: Record<number, string[]> = {
    0: [],
    1: ['axes', 'curve-upper', 'curve-lower'],
    2: ['axes', 'curve-upper', 'curve-lower', 'point-p'],
    3: ['axes', 'curve-upper', 'curve-lower', 'point-p', 'tangent', 'point-rprime'],
    4: ['axes', 'curve-upper', 'curve-lower', 'point-p', 'tangent', 'point-rprime', 'reflect-line', 'point-r'],
    5: ['axes', 'curve-upper', 'curve-lower', 'point-p', 'tangent', 'point-rprime', 'reflect-line', 'point-r', 'result']
  };
  ...
  onMount(() => {
    ...
    // Tangent line (extend slightly past P and R')
    const lineStartX = P.x - 0.5;
    const lineEndX = Rprime.x + 0.5;
    const lineStart = toSvg(lineStartX, m * lineStartX + c);
    const lineEnd = toSvg(lineEndX, m * lineEndX + c);

    svgEl.append('line').attr('class', 'tangent')
    ...
    pointR.append('text').attr('x', rSvg.sx + 18).attr('y', rSvg.sy + 5)
      .attr('fill', colors.green).attr('font-size', '16px').attr('font-weight', 'bold').text('2P');
    ...
    result.append('text').attr('x', 500).attr('y', 47).attr('text-anchor', 'middle')
      .attr('fill', colors.green).attr('font-size', '16px').attr('font-weight', 'bold').text('2P = P + P');
