<script lang="ts">
	...
	onMount(() => {
		...
		for (let i = 0; i < bits.length; i++) {
			...
			// Phase indicator (small text showing 0° or 180°)
			svgEl.append('text')
				.attr('class', `phase-${i}`)
				.attr('x', x + bitWidth / 2)
				.attr('y', 78)
				.attr('text-anchor', 'middle')
				.attr('fill', bits[i] === 1 ? colors.accent : '#fb4934')
				.attr('font-size', '9px')
				.text(bits[i] === 1 ? '0°' : '180°')
				.attr('opacity', 0);
		...
		svgEl.append('text')
			...
			.attr('fill', colors.fgMuted)
			.attr('font-size', '11px')
			.text('1 = 0° phase  •  0 = 180° phase (flipped)');
