<script lang="ts">
  import { onMount, onDestroy } from 'svelte';
  import * as d3 from 'd3';
  import type { AnimationController } from '$lib/components/AnimationWrapper.svelte';

  let { register }: { register?: (controller: AnimationController) => void } = $props();
  let svg: SVGSVGElement;
  let svgEl: d3.Selection<SVGSVGElement, unknown, null, undefined>;
  let timeouts: ReturnType<typeof setTimeout>[] = [];
  let colors: Record<string, string>;
  let animationRunning = true;

  const maxStep = 0;
  let isPlaying = $state(false);
  let currentStep = $state(0);

  function sleep(ms: number): Promise<void> {
    return new Promise((r) => { timeouts.push(setTimeout(r, ms)); });
  }
  function clearTimeouts() { timeouts.forEach(clearTimeout); timeouts = []; }

  function applyStep(step: number) { currentStep = step; }
  async function runAnimation() { if (!isPlaying) return; isPlaying = false; }
  function play() { if (!isPlaying) { isPlaying = true; runAnimation(); } }
  function pause() { isPlaying = false; clearTimeouts(); }
  function next() { pause(); if (currentStep < maxStep) applyStep(currentStep + 1); }
  function prev() { pause(); if (currentStep > 0) applyStep(currentStep - 1); }
  function skip() { pause(); applyStep(maxStep); }
  function replay() { pause(); currentStep = 0; applyStep(0); }
  function getState() { return { isPlaying, currentStep, totalSteps: maxStep }; }

  onMount(() => {
    const s = getComputedStyle(document.documentElement);
    colors = {
      bg: s.getPropertyValue('--color-bg').trim(),
      fg: s.getPropertyValue('--color-fg').trim(),
      accent: s.getPropertyValue('--color-accent').trim(),
      yellow: s.getPropertyValue('--color-math').trim(),
      muted: s.getPropertyValue('--color-fg-muted').trim(),
    };

    const width = 400;
    const height = 380;
    const cx = width / 2;
    const cy = height / 2 + 10;
    const radius = 120;

    svgEl = d3.select(svg).attr('viewBox', `0 0 ${width} ${height}`);

    // Title
    svgEl.append('text')
      .attr('x', cx).attr('y', 28)
      .attr('fill', colors.fg).attr('font-size', '15px')
      .attr('text-anchor', 'middle')
      .text('Signs by quadrant');

    // Axes
    svgEl.append('line')
      .attr('x1', cx - radius - 20).attr('y1', cy)
      .attr('x2', cx + radius + 20).attr('y2', cy)
      .attr('stroke', colors.muted).attr('stroke-width', 1.5);

    svgEl.append('line')
      .attr('x1', cx).attr('y1', cy + radius + 20)
      .attr('x2', cx).attr('y2', cy - radius - 20)
      .attr('stroke', colors.muted).attr('stroke-width', 1.5);

    // Axis labels
    svgEl.append('text')
      .attr('x', cx + radius + 25).attr('y', cy + 5)
      .attr('fill', colors.muted).attr('font-size', '13px')
      .text('+x');

    svgEl.append('text')
      .attr('x', cx - radius - 30).attr('y', cy + 5)
      .attr('fill', colors.muted).attr('font-size', '13px')
      .text('-x');

    svgEl.append('text')
      .attr('x', cx + 5).attr('y', cy - radius - 25)
      .attr('fill', colors.muted).attr('font-size', '13px')
      .text('+y');

    svgEl.append('text')
      .attr('x', cx + 5).attr('y', cy + radius + 35)
      .attr('fill', colors.muted).attr('font-size', '13px')
      .text('-y');

    // Circle
    svgEl.append('circle')
      .attr('cx', cx).attr('cy', cy).attr('r', radius)
      .attr('fill', 'none')
      .attr('stroke', colors.fg)
      .attr('stroke-width', 2);

    // Quadrant labels
    const quadrants = [
      { x: cx + radius / 2, y: cy - radius / 2, signs: ['cos +', 'sin +'], num: 'I' },
      { x: cx - radius / 2, y: cy - radius / 2, signs: ['cos -', 'sin +'], num: 'II' },
      { x: cx - radius / 2, y: cy + radius / 2, signs: ['cos -', 'sin -'], num: 'III' },
      { x: cx + radius / 2, y: cy + radius / 2, signs: ['cos +', 'sin -'], num: 'IV' },
    ];

    quadrants.forEach(q => {
      // Quadrant number
      svgEl.append('text')
        .attr('x', q.x).attr('y', q.y - 15)
        .attr('fill', colors.accent)
        .attr('font-size', '16px')
        .attr('font-weight', 'bold')
        .attr('text-anchor', 'middle')
        .text(q.num);

      // cos sign
      svgEl.append('text')
        .attr('x', q.x).attr('y', q.y + 8)
        .attr('fill', q.signs[0].includes('+') ? colors.yellow : colors.muted)
        .attr('font-size', '13px')
        .attr('text-anchor', 'middle')
        .text(q.signs[0]);

      // sin sign
      svgEl.append('text')
        .attr('x', q.x).attr('y', q.y + 26)
        .attr('fill', q.signs[1].includes('+') ? colors.yellow : colors.muted)
        .attr('font-size', '13px')
        .attr('text-anchor', 'middle')
        .text(q.signs[1]);
    });

    // Animated point
    const point = svgEl.append('circle')
      .attr('r', 6)
      .attr('fill', colors.accent);

    const radiusLine = svgEl.append('line')
      .attr('x1', cx).attr('y1', cy)
      .attr('stroke', colors.accent)
      .attr('stroke-width', 2);

    async function animate() {
      let angle = 0;
      while (animationRunning) {
        const rad = -angle * Math.PI / 180;
        const px = cx + radius * Math.cos(rad);
        const py = cy + radius * Math.sin(rad);

        point.attr('cx', px).attr('cy', py);
        radiusLine.attr('x2', px).attr('y2', py);

        angle = (angle + 0.8) % 360;
        await sleep(30);
      }
    }

    animate();
    register?.({ play, pause, next, prev, skip, replay, getState });
  });

  onDestroy(() => { animationRunning = false; clearTimeouts(); });
</script>

<svg bind:this={svg} class="diagram"></svg>

<style>
  .diagram { display: block; margin: 0 auto; background: var(--color-bg); border-radius: 0.5rem; width: 100%; max-width: 400px; }
</style>
