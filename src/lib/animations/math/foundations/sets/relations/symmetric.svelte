<script lang="ts">
	...
	let display = $derived.by(() => {
		...
		if (currentStep === 0) {
			message = 'Two elements a and b';
		} else if (currentStep === 1) {
			showForward = true;
			message = 'If a relates to b...';
		} else if (currentStep === 2) {
			showForward = true;
			showBackward = true;
			message = '...then b must relate to a';
		}

		return { showForward, showBackward, message };
	});
		...
		<svg viewBox="0 0 280 160" class="diagram">
			...
			{#if display.showForward}
				<path
					...
					class="arrow"
					marker-end="url(#arrowhead-symmetric)"
				/>
			{/if}
