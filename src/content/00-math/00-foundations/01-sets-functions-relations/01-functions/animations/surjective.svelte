	...
	let display = $derived.by(() => {
		if (currentStep === 0) {
			...
			return {
				title: 'Surjective function',
				showArrows: true,
				message: 'Every output is hit (Range = Codomain)'
			};
		}
	});
	...
	<div class="diagram-container">
		<svg viewBox="0 0 300 160" class="diagram">
			...
			{#each codomain as el, i}
				<circle cx={rightX} cy={getRightY(i)} r="14" class="element-circle" />
				<text x={rightX} y={getRightY(i) + 5} class="element-text">{el}</text>
			{/each}
