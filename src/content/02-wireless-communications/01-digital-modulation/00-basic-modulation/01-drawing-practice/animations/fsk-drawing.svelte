<script lang="ts">
	...
	function applyStep(step: number) {
		...
		const msg = step === 0
			? 'Draw FSK: high frequency for 1, low frequency for 0'
			: step <= bits.length
				? `Bit ${step}: ${bits[step - 1]} → ${bits[step - 1] === 1 ? 'HIGH frequency' : 'LOW frequency'}`
				: 'Complete FSK waveform';
	...
	onMount(() => {
		...
		bits.forEach((bit, i) => {
			...
			const cycles = bit === 1 ? 6 : 2; // High freq = more cycles

			let pathD = '';
			const points = 40;
			for (let p = 0; p <= points; p++) {
				const px = x + (p / points) * bitWidth;
				const py = waveY - Math.sin((p / points) * Math.PI * 2 * cycles) * amplitude;
				pathD += (p === 0 ? 'M' : 'L') + ` ${px} ${py}`;
			}
		...
		svgEl.append('text')
			...
			.attr('fill', colors.fg)
			.attr('font-size', '12px')
			.text('Draw FSK: high frequency for 1, low frequency for 0');
