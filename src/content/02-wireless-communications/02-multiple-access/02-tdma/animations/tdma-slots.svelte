<script lang="ts">
	...
	const slotColors = ['#fb4934', '#b8bb26', '#83a598', '#d3869b'];
	const numSlots = 4;
	...
	function applyStep(step: number) {
		...
		const messages = [
			...
			'Each frame has multiple time slots',
			'Each user gets one slot per frame',
			'Only one user transmits at a time',
			'Users take turns, frame after frame!'
		];
	...
	function updateActiveSlot() {
		...
		svgEl.select('.transmitting-label')
			.text(`User ${activeSlot + 1} transmitting`);
		...
		svgEl.selectAll('.user-phone').each(function(_, i) {
			d3.select(this)
				...
				.attr('stroke-width', i === activeSlot ? 3 : 0);
		});
	...
	onMount(() => {
		...
		const slotGap = 10;
		const frameWidth = numSlots * slotWidth + (numSlots - 1) * slotGap;
		const slotStartX = (width - frameWidth) / 2; // Center the frame

		// Timeline background
		svgEl.append('rect')
			.attr('class', 'timeline-bg')
			.attr('x', slotStartX - 10)
			.attr('y', timelineY - 5)
			.attr('width', frameWidth + 20)
			.attr('height', slotHeight + 10)
		...
		svgEl.append('text')
			...
			.attr('x', slotStartX + frameWidth / 2)
			.attr('y', timelineY - 15)
			.attr('text-anchor', 'middle')
		...
		svgEl.append('text')
			.attr('class', 'frame-label')
			.attr('x', slotStartX + frameWidth + 25)
			.attr('y', timelineY + slotHeight / 2 + 4)
		...
		svgEl.append('rect')
			...
			.attr('x', slotStartX - 3)
			.attr('y', timelineY - 3)
			.attr('width', slotWidth + 6)
			.attr('height', slotHeight + 6)
			.attr('fill', 'none')
			.attr('stroke', '#fabd2f')
			.attr('stroke-width', 3)
			.attr('rx', 6)
		...
		for (let i = 0; i < numSlots; i++) {
			...
			svgEl.append('text')
				...
				.attr('y', timelineY + slotHeight / 2 + 5)
				.attr('text-anchor', 'middle')
				.attr('fill', colors.bg)
				.attr('font-size', '11px')
				.attr('font-weight', 'bold')
			...
			// Slot divider (guard time)
			if (i < numSlots - 1) {
				svgEl.append('line')
					.attr('class', 'slot-divider')
					.attr('x1', x + slotWidth + slotGap / 2)
					.attr('x2', x + slotWidth + slotGap / 2)
					.attr('y1', timelineY)
					.attr('y2', timelineY + slotHeight)
		...
		const userY = timelineY + slotHeight + 60;
		for (let i = 0; i < numSlots; i++) {
			...
			[8, 14, 20].forEach((r) => {
				burstGroup.append('path')
					.attr('d', `M ${x - r} ${userY - 25} A ${r} ${r} 0 0 1 ${x + r} ${userY - 25}`)
					.attr('fill', 'none')
					.attr('stroke', slotColors[i])
					.attr('stroke-width', 2)
					.attr('opacity', 0.8 - r * 0.02);
			});
		}
		...
		svgEl.append('text')
			...
			.attr('x', width / 2)
			.attr('y', timelineY + slotHeight + 115)
			.attr('text-anchor', 'middle')
			.attr('fill', '#fabd2f')
			.attr('font-size', '12px')
			.attr('font-weight', 'bold')
			.text('User 1 transmitting')
			.attr('opacity', 0);
		...
		svgEl.append('text')
			...
			.attr('fill', colors.fg)
			.attr('font-size', '12px')
			.text('Time is divided into frames');
